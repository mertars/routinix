...
function taskFieldGuide(category) {
  ...
  if (category === "fitness") {
    return `Token tasarrufu için her günün "tasks" alanı NESNE DİZİSİ DEĞİL, tam olarak şu SIRADA 5 elemanlı bir dizi-içinde-dizi olmalı: [title, description, duration_min, priority, active_widgets]. title: egzersiz/aktivite adı. description: kısa form/biyomekanik ipucu — set sayısını, tekrar aralığını (örn. "8-12") VE hedef RPE/RIR'ı (örn. "RPE 8") DOĞAL bir cümleye göm, ayrı alan AÇMA (örn. "3x10, RPE 8 - dirsekleri sabit tut, kontrollü indir"). duration_min: tahmini süre (dakika). priority: "Yüksek"|"Orta"|"Düşük" (egzersizin programdaki önemine göre). active_widgets: SADECE gerçek bir set/tekrar içeren AĞIRLIK ANTRENMANI görevinde (dinlenme/kardiyo/esneme günlerinde BOŞ dizi []) tam olarak şu şekilde TEK elemanlı bir dizi: [{"type":"sets_reps","value":{"sets":4,"repRange":"8-10","rpe":8}}] — sets/repRange/rpe DEĞERLERİ description'daki İLE BİREBİR AYNI olmalı (uydurma/farklı bir sayı YAZMA), weightKg alanı VERME (kullanıcı kendi girer).\n\n${WHY_IT_WORKS_RULE}`;
  }
