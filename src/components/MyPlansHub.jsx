...
export default function MyPlansHub({ open, userId, onOpenPlan, onClose }) {
  ...
  return (
    ...
    >
      {/* Üst bar */}
      <div className="shrink-0 px-4 md:px-8 pt-5 pb-4 border-b border-white/8">
        <div className="max-w-6xl mx-auto w-full flex items-center justify-between">
          <div>
            <h2 className="text-[18px] md:text-[22px] font-bold text-slate-100">📂 Planlarım</h2>
            <p className="text-[12px] text-slate-400 mt-0.5">{list.length} aktif plan</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Kapat"
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-5">
        <div className="max-w-6xl mx-auto w-full">
          {loading && !plans ? (
            <p className="text-[13px] text-slate-400 py-10 text-center font-medium">Yükleniyor...</p>
          ) : list.length === 0 ? (
            <p className="text-[13px] text-slate-400 py-10 text-center font-medium">Henüz kayıtlı bir planın yok.</p>
          ) : (
              ...
              {list.map((p) => {
                ...
                return (
                  <button
                    ...
                    className="glass rounded-2xl p-4 text-left flex flex-col gap-3 card-glow transition-colors hover:bg-white/[0.04]"
                    style={{ borderColor: `${cat.accent}30` }}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="text-[20px] shrink-0">{cat.emoji}</span>
                        <h3 className="text-[14px] font-semibold text-slate-100 leading-snug truncate">{p.title || "Plan"}</h3>
                      </div>
                      {p.total_days ? (
                        <span
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[11.5px] font-semibold text-slate-300" style={{ fontFamily: "ui-monospace, monospace" }}>
                          {done}/{total} Görev
                        </span>
